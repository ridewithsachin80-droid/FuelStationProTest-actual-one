...
// BUG-22 FIX: Use collision-resistant ID generator instead of bare Date.now()
// Two records created within same ms would get the same ID → silent DB overwrite.
function _genId() {
...
function saveAdvance() {
  ...
  if (!window._advancesData) window._advancesData = [];
  window._advancesData.unshift({ id: _genId(), empId, empName: emp?.name||'', amount, reason, repayMonths, monthlyEmi, balance: amount, date, status:'active' });
...
function wa_log(type, message, status) {
  window._waLog.unshift({ id: _genId(), type, message: message.slice(0,200), status, ts: new Date().toLocaleString('en-IN') });
...
function calcLpCarton() {
  ...
  const mrpPack  = parseFloat(document.getElementById('lp_mrp')?.value) || 0;
  ...
  if (marginBox) {
    if (qpc > 0 && costCar > 0) {
      ...
      if (mt) mt.innerHTML = '₹' + costCar.toFixed(2) + ' ÷ ' + qpc + ' = <strong>₹' + costPc.toFixed(2) + ' cost/pc</strong>'
        + (mrpPack > 0 && qpc > 0 ? ' &nbsp;·&nbsp; MRP/pc: <strong style="color:var(--text-1)">₹' + (mrpPack/qpc).toFixed(2) + '</strong>' : '')
...
function openAddLubeModal(prefill) {
  ...
  setTimeout(()=>{
    if (isCarton) {
      const sellEl = document.getElementById('lp_sell');
      if (sellEl && (parseFloat(sellEl.value) || 0) === 0) sellEl.dataset.autoFilled = '1';
      calcLpCarton();
    }
  }, 80);
}
...
function openEditLubeModal(id) {
  ...
  setTimeout(()=>{
    if (isCarton) {
      // If selling price is 0 or unset, mark as auto-fillable so calcLpCarton populates it from MRP
      const sellEl = document.getElementById('lp_sell');
      if (sellEl && (parseFloat(sellEl.value) || 0) === 0) sellEl.dataset.autoFilled = '1';
      calcLpCarton();
    }
...
async function saveAddAdminUser() {
  ...
  if (typeof TenantAPI !== 'undefined' && APP.tenant?.id) {
    ...
    APP.tenant.adminUsers.push({ id: _genId(), name, username: user, passHash, role });
    toast(`${name} added as ${role}`, 'success');
    closeModal(); renderPage();
  }
}
...
function openEditAdminUserRoleModal(userIdx) {
  ...
  `, `<button class="btn btn-ghost" onclick="closeModal()">Cancel</button>
      <button class="btn btn-accent" onclick="saveAdminUserRole(${userIdx})">Save</button>
...
async function saveExpense() {
  ...
  const expense = { id: _genId(), date: today(), category: sanitize(category), amount, desc: sanitize(finalDesc), ...meta };
...
async function savePurchase() {
  ...
  const purchase  = {
    id: _genId(), date: today(), fuelType, liters,
  ...
  const fuelInfo = getFuel(fuelType);
  const expEntry = {
    id: _genId() + 1, date: today(), category: 'Fuel Purchase',
...
async function addEmployee() {
  ...
  const newEmp = {
    id: _genId(), name, role, shift, phone: phone || '', salary,
  ...
  try {
    ...
    if (pinSetOk) {
      ...
      // BUG-17 FIX: If PIN setup failed, warn admin clearly so they know to set it manually
      toast(`${name} added — ⚠️ PIN could not be set automatically. Go to Staff Directory → 🔑 Set PIN before this employee can log in.`, 'warning');
    }
    renderPage();
  } catch (e) {
