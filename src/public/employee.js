...
// ── ONE-TIME MIGRATION: Clear stale SHA-256 PIN cache ────────────────────
(function _clearStalePinCache() {
  try {
    const VERSION_KEY = 'fb_pin_cache_v';
    const CURRENT_VERSION = '2';
    const storedVersion = localStorage.getItem(VERSION_KEY);
    if (storedVersion !== CURRENT_VERSION) {
      localStorage.removeItem('fb_emp_pins');
      localStorage.setItem(VERSION_KEY, CURRENT_VERSION);
      console.log('[PINMigration] Cleared stale SHA-256 PIN cache → server bcrypt verification active');
    }
  } catch(e) {}
})();

// ── CROSS-TENANT FIX: Tenant-scoped localStorage keys ─────────────────────
// CRITICAL BUG: All localStorage keys were flat (e.g. 'fb_emp_cache') with no
// tenant ID. When a user visited Station A then Station B, Station B showed
// Station A's employees in the login dropdown — a serious cross-tenant data leak.
//
// FIX: All tenant-specific data is now keyed as 'fb_emp_cache_{tenantId}'.
// Data from one station never appears in another station's login screen.
...
// Cache the active tenant ID in a simple key so _tenantKey works before mt_getActiveTenant loads
function _cacheTenantId() {
  try {
    const t = (typeof mt_getActiveTenant === 'function') ? mt_getActiveTenant() : null;
    if (t?.id) localStorage.setItem('fb_active_tenant_id', t.id);
  } catch {}
}
...
function mt_doSwitchStation() {
  ...
  try { localStorage.removeItem(_tenantKey('fb_emp_session')); } catch {}
...
function loadEmpPins() {
  try { return JSON.parse(localStorage.getItem(_tenantKey('fb_emp_pins')) || '{}'); } catch { return {}; }
}
function saveEmpPins(pins) {
  try { localStorage.setItem(_tenantKey('fb_emp_pins'), JSON.stringify(pins)); } catch {}
}
...
function getEmpList() {
  ...
  try {
    const cached = JSON.parse(localStorage.getItem(_tenantKey('fb_emp_cache')) || '[]');
    if (cached.length > 0) {
      return cached.map(e => ({
        id: e.id, name: e.name, role: e.role, shift: e.shift || '',
        permissions: e.permissions || {},
...
function emp_loadHistory() { try { return JSON.parse(localStorage.getItem(_tenantKey('fb_emp_history')) || '[]'); } catch { return []; } }
function emp_saveHistory() {
  try { localStorage.setItem(_tenantKey('fb_emp_history'), JSON.stringify(empState.sessionHistory.slice(0, 50))); } catch {}
}
...
function emp_loadSession() {
  try {
    const raw = localStorage.getItem(_tenantKey('fb_emp_session'));
    if (!raw) return false;
    const s = verifyData(raw);
    if (!s || !validateEmpSessionShape(s)) { localStorage.removeItem(_tenantKey('fb_emp_session')); return false; }
    empState = { ...empState, ...s };
    return true;
  } catch { return false; }
}
function emp_saveSession() {
  try {
    ...
    localStorage.setItem(_tenantKey('fb_emp_session'), signData(data));
  } catch {}
}
...
async function emp_completeLogin(emp, isPinLogin) {
  ...
  try {
    const rawS = localStorage.getItem(_tenantKey('fb_emp_session'));
    if (rawS) {
  ...
  if (navigator.onLine && tenant2 && tenant2.id) {
    try {
      ...
      if (verifyResp.ok) {
        ...
        if (result.locked) {
          toast(result.error || 'Account locked — too many failed attempts. Wait 15 minutes.', 'error');
          return;
        }
        pinValid = result.valid === true;
      } else {
        // Server HTTP error — fall back to local hash
        const localHash = emp.pinHash;
        pinValid = (localHash && localHash !== '__server__' && !localHash.startsWith('$2'))
          ? pinHash === localHash : false;
      }
    } catch {
      // Network failure — fall back to local hash
      const localHash = emp.pinHash;
      pinValid = (localHash && localHash !== '__server__' && !localHash.startsWith('$2'))
        ? pinHash === localHash : false;
    }
  } else {
    // Offline — use locally cached SHA-256 hash only
    const localHash = emp.pinHash;
    if (localHash && localHash !== '__server__' && !localHash.startsWith('$2')) {
      pinValid = pinHash === localHash;
    } else {
      toast('You are offline and no local PIN is cached. Connect to the internet to login.', 'error');
      return;
    }
  }
...
async function fetchPublicEmployees() {
  ...
  try {
    ...
    if (Array.isArray(emps) && emps.length > 0) {
      ...
      try { localStorage.setItem(_tenantKey('fb_emp_cache'), JSON.stringify(emps)); } catch {}
...
async function doEmpLogin() {
  ...
  if (!emp) {
    try {
      var cached = JSON.parse(localStorage.getItem(_tenantKey('fb_emp_cache')) || '[]');
  ...
  // ── PERMANENT PIN FIX ─────────────────────────────────────────────────────
  // ROOT CAUSE of "Incorrect PIN" after bcrypt migration:
  // The old code checked local SHA-256 hash FIRST (from fb_emp_pins localStorage).
  // After migration, server stores bcrypt but local cache has stale SHA-256.
  // SHA-256("1234") never equals bcrypt("1234") → always failed locally without
  // ever calling the server.
  //
  // PERMANENT SOLUTION:
  // 1. ALWAYS try server verification first (bcrypt.compare — always correct)
  // 2. Only fall back to local SHA-256 when genuinely offline AND local hash exists
  // 3. Never use local hash when online — server is the source of truth
  // ──────────────────────────────────────────────────────────────────────────
  var pinHash = await hashPassword(pin); // SHA-256 — only for true offline fallback
  var pinValid = false;
  var tenant = (typeof mt_getActiveTenant === 'function') ? mt_getActiveTenant() : null;

  if (navigator.onLine && tenant && tenant.id) {
    // ONLINE PATH: always verify via server (bcrypt) — regardless of local hash
    try {
      ...
      if (vResult.locked) {
        toast(vResult.error || 'Account locked — too many failed attempts. Wait 15 minutes.', 'error');
        return;
      }
...
async function resetData() {
  ...
  try {
    localStorage.removeItem(_tenantKey('fb_allocations'));
    localStorage.removeItem(_tenantKey('fb_emp_cache'));
    localStorage.removeItem(_tenantKey('fb_alloc_shift'));
  } catch {}
  ...
  try {
    ...
    try {
      ...
      if (empCacheData.length > 0) {
        localStorage.setItem(_tenantKey('fb_emp_cache'), JSON.stringify(empCacheData));
      }
    } catch {}
