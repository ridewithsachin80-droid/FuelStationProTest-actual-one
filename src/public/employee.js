...
// After bcrypt migration, fb_emp_pins stores SHA-256 hashes which no longer
// match server bcrypt hashes. The login flow now always verifies via server
// when online, so local PIN cache is not needed. Clear it once so the stale
// SHA-256 hashes stop blocking login via the old local-hash-first code path.
(function _clearStalePinCache() {
  try {
    const VERSION_KEY = 'fb_pin_cache_v';
    const CURRENT_VERSION = '2'; // bump this to force clear again if needed
